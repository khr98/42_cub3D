...
void			draw_sprite_ud(t_screen *scr)
{
	scr->ray_sprite.height = abs((int)(scr->screen_height
				/ scr->ray_sprite.transY));
	scr->ray_sprite.drawStartY = -scr->ray_sprite.height
		/ 2 + scr->screen_height / 2;
	if (scr->ray_sprite.drawStartY < 0)
		scr->ray_sprite.drawStartY = 0;
	scr->ray_sprite.drawEndY = scr->ray_sprite.height
		/ 2 + scr->screen_height / 2;
	...
	scr->ray_sprite.width = abs((int)(scr->screen_height
				/ scr->ray_sprite.transY));
	scr->ray_sprite.drawStartX = -scr->ray_sprite.width
		/ 2 + scr->ray_sprite.screenX;
	if (scr->ray_sprite.drawStartX < 0)
		scr->ray_sprite.drawStartX = 0;
	scr->ray_sprite.drawEndX = scr->ray_sprite.width
		/ 2 + scr->ray_sprite.screenX;
...
void			draw_sprite(t_screen *scr, int stripe)
{
	...
	scr->ray_sprite.texX = (int)((256 * (stripe - (-scr->ray_sprite.width / 2
						+ scr->ray_sprite.screenX)) * TEX_WIDTH
				/ scr->ray_sprite.width) / 256);
	if (scr->ray_sprite.transY > 0 && stripe > 0 && stripe < scr->screen_width
			&& scr->ray_sprite.transY < scr->z_buf[stripe])
	{
		...
		while (y < scr->ray_sprite.drawEndY)
		{
			...
			scr->ray_sprite.texY = ((d * TEX_HEIGHT)
					/ scr->ray_sprite.height) / 256;
			color = scr->texture[4][TEX_WIDTH * scr->ray_sprite.texY
				+ scr->ray_sprite.texX];
...
void			sprite_location(t_screen *scr, int i)
{
		...
		scr->ray_sprite.transX = scr->ray_sprite.inverse * (scr->player.dirY
				* scr->ray_sprite.rDistX - scr->player.dirX
				* scr->ray_sprite.rDistY);
		scr->ray_sprite.transY = scr->ray_sprite.inverse * (-scr->player.planeY
				* scr->ray_sprite.rDistX + scr->player.planeX
				* scr->ray_sprite.rDistY);
